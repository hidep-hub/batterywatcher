...
![タスクトレイに常時表示されるバッテリー残量](docs/images/tray-icon.png)

いちいちマウスを乗せてツールチップを確認しなくても、タスクトレイを見た瞬間に残量が分かる。この「いつも見えている」安心感が一番の売り。しきい値（緑/黄/赤の境界）は設定画面で自分好みに調整できるので、「あと何%まで踏ん張れるか」の感覚に合わせて、バッテリーを限界まで使い切りたい人ほど活躍する。
...
3. タスクトレイに常駐アイコンが表示されれば起動完了（メインウィンドウは表示されない仕様）。
   - 初回はWindowsの仕様でアイコンが「隠れているインジケーター」（通知領域の `^` をクリックすると出てくる領域）に収納され、パッと見えない場合がある。常に見える位置に出すには、タスクバーを右クリック→「タスクバーの設定」→「タスクバーに表示するアイコンを選択」（Windows 11では「他のシステムトレイアイコン」）を開き、BatteryWatcherを「オン」にする。
...
![右クリック＞設定](./docs/images/setting.png)
...
「赤」しきい値未満になると、こんな感じで点滅表示に切り替わる。

![残量が「赤」しきい値未満になったときの点滅表示](./docs/images/batteryalert.gif)
